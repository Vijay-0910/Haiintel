...
const RichContentRenderer = memo(({ message, isDarkMode }) => (
  <>
    {message.stats && <StatsCard stats={message.stats} isDarkMode={isDarkMode} />}
    {message.image && <ImageDisplay image={message.image} isDarkMode={isDarkMode} />}
    {message.chart && <BarChart data={message.chart} isDarkMode={isDarkMode} />}
    {message.details && <DetailCard details={message.details} isDarkMode={isDarkMode} />}
    {message.list && <ListDisplay items={message.list} ordered={message.listOrdered} isDarkMode={isDarkMode} />}
  </>
));
RichContentRenderer.displayName = "RichContentRenderer";

// Regenerate Button
const RegenerateButton = memo(({ onRegenerate, isDarkMode }) => {
  return (
    <button
...
const MessageBubble = memo(({ message, isStreaming = false, isDarkMode = true, onRegenerate, onRetry, onOpenArtifacts }) => {
  ...
  const hasRichContent = useMemo(
    () => message.stats || message.image || message.chart || message.details || message.list,
    [message.stats, message.image, message.chart, message.details, message.list]
  );
  ...
  return (
    <div
      className={`group flex items-start gap-3 mb-6 w-full animate-fade-in ${isUser ? "justify-end" : "justify-start"}`}
      style={{ contentVisibility: 'auto', containIntrinsicSize: '0 100px' }}
    >
      {!isUser && <AIAvatar />}

      <div className={`flex flex-col gap-2 ${isUser ? "items-end max-w-[85%]" : "items-start flex-1"} min-w-0`}>
        ...
        <div className={bubbleClassName}>
          {/* Images */}
          {hasImages && (
            <div className={`grid gap-1.5 sm:gap-2 ${message.images.length === 1 ? 'grid-cols-1' : message.images.length === 2 ? 'grid-cols-2' : 'grid-cols-2'} ${message.text ? 'mb-2' : ''}`}>
              {message.images.map((image, index) => (
                <div
                  key={image.id || index}
                  className="relative rounded-lg overflow-hidden cursor-pointer hover:opacity-90 transition-opacity"
                  onClick={() => setLightboxImage(image)}
                  style={{ maxWidth: message.images.length === 1 ? '300px' : '150px' }}
                >
                  <img
                    src={image.data}
                    alt={image.name || `Image ${index + 1}`}
                    className="w-full h-auto object-cover rounded-lg"
                    style={{ maxHeight: message.images.length === 1 ? '300px' : '150px' }}
                  />
                </div>
              ))}
            </div>
              ...
              {isUser ? (
                ...
                <div className="text-[15px] leading-[1.7]">
                  {isStreaming && !isComplete ? (
                    ...
                    <MarkdownMessage content={textToShow} isDarkMode={isDarkMode} />
                  )}
                </div>
              )}
            </>
          ...
          {(!isStreaming || isComplete) && hasRichContent && (
            <Suspense fallback={<RichContentLoader />}>
              <RichContentRenderer message={message} isDarkMode={isDarkMode} />
            </Suspense>
          )}
        </div>
