...
const LineChart = memo(({ data, isDarkMode = true }) => {
  ...
  const downloadAsSVG = useCallback(() => {
    ...
    // Draw axes
    svg += `<line x1="${padding}" y1="${height - padding}" x2="${
      width - padding
    }" y2="${height - padding}" stroke="${
      isDarkMode ? "#666" : "#ccc"
    }" stroke-width="2"/>`;
    svg += `<line x1="${padding}" y1="${padding}" x2="${padding}" y2="${
      height - padding
    }" stroke="${isDarkMode ? "#666" : "#ccc"}" stroke-width="2"/>`;
    ...
    data.values.forEach((value, index) => {
      ...
      svg += `<text x="${x}" y="${
        ...
        isDarkMode ? "#999" : "#666"
      }" font-size="12">${data.labels[index]}</text>`;

      // Draw values
      svg += `<text x="${x}" y="${
        y - 10
      }" text-anchor="middle" fill="#2e90fa" font-size="12" font-weight="bold">${value}</text>`;
    });
  ...
  }, [data, isDarkMode, minValue, range]);
