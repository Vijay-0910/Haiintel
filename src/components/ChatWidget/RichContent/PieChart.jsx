...
const PieChart = memo(({ data, isDarkMode = true }) => {
  ...
  const colors = [
    '#2e90fa', '#00d4ff', '#8b5cf6', '#f59e0b', '#10b981',
  ...
  const downloadAsSVG = useCallback(() => {
    ...
    let svg = `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
      <rect width="${size}" height="${size}" fill="${isDarkMode ? '#1a1a1a' : '#ffffff'}"/>
    ...
    data.labels.forEach((label, index) => {
      svg += `<rect x="20" y="${legendY + index * 25}" width="15" height="15" fill="${colors[index % colors.length]}"/>`;
      svg += `<text x="40" y="${legendY + index * 25 + 12}" fill="${isDarkMode ? '#999' : '#666'}" font-size="12">${label}: ${data.values[index]} (${((data.values[index] / total) * 100).toFixed(1)}%)</text>`;
    });
  ...
  const downloadAsPNG = useCallback(() => {
    ...
    data.labels.forEach((label, index) => {
      ctx.fillStyle = colors[index % colors.length];
      ctx.fillRect(30, legendY + index * 30, 20, 20);

      ctx.fillStyle = isDarkMode ? '#999999' : '#666666';
      ctx.font = '14px Arial';
      ctx.textAlign = 'left';
      const percentage = ((data.values[index] / total) * 100).toFixed(1);
      ctx.fillText(`${label}: ${data.values[index]} (${percentage}%)`, 60, legendY + index * 30 + 15);
    });
