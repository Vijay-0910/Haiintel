...
const BarChart = memo(({ data, isDarkMode = true }) => {
  ...
  const downloadAsSVG = useCallback(() => {
    ...
    data.labels.forEach((label, index) => {
      ...
      // Bar background
      svg += `<rect x="${padding}" y="${y}" width="${width - padding * 2}" height="${barHeight}" fill="${isDarkMode ? '#333' : '#f0f0f0'}" rx="4"/>`;
      // Bar
      svg += `<rect x="${padding}" y="${y}" width="${barWidth}" height="${barHeight}" fill="#2e90fa" rx="4"/>`;
      // Label
      svg += `<text x="${padding - 10}" y="${y + barHeight/2 + 5}" text-anchor="end" fill="${isDarkMode ? '#999' : '#666'}" font-size="12">${label}</text>`;
      // Value
      svg += `<text x="${padding + barWidth + 10}" y="${y + barHeight/2 + 5}" fill="#2e90fa" font-size="12" font-weight="bold">${value}</text>`;
    });
  ...
  const downloadAsPNG = useCallback(() => {
    ...
    data.labels.forEach((label, index) => {
      ...
      ctx.textAlign = 'left';
      ctx.fillText(value.toString(), padding + barWidth + 15, y + barHeight / 2 + 5);
    });
