...
const ChatInput = memo(({ onSend, onStop, disabled = false, isStreaming = false, isDarkMode = true }) => {
  ...
  const handleFileSelect = useCallback((e) => {
    console.log('File select triggered', { filesCount: e.target.files?.length });
  ...
  const handleImageClick = useCallback(() => {
    console.log('Image button clicked', { disabled, hasRef: !!fileInputRef.current });
  ...
  return (
    <div className={`px-2 py-2 sm:px-3 sm:py-2.5 md:px-4 md:py-3 flex-shrink-0 ${isDarkMode ? "bg-haiintel-darker" : "bg-gray-50"}`}>
      {/* Image previews */}
      ...
      <form onSubmit={handleSubmit}>
        <div
          className={`relative flex items-end gap-1.5 sm:gap-2 rounded-xl sm:rounded-2xl border transition-all duration-150 px-2 py-1.5 sm:px-3 sm:py-2 ${
            isDarkMode
              ? `bg-haiintel-dark/80 ${isFocused ? "border-haiintel-blue/50 ring-1 ring-haiintel-blue/20" : "border-haiintel-border"}`
              : `bg-white ${isFocused ? "border-haiintel-blue/50 ring-1 ring-haiintel-blue/20 shadow-sm" : "border-gray-300"}`
          }`}
        >
          {/* Hidden file input */}
          <input
          ...
          <div className="flex-1 min-w-0">
            <textarea
              ...
              className={`w-full bg-transparent text-xs sm:text-sm focus:outline-none resize-none scrollbar-hide leading-5 sm:leading-6 ${
                isDarkMode ? "text-haiintel-text placeholder-gray-500" : "text-gray-900 placeholder-gray-400"
              }`}
        ...
        {/* Footer hint */}
        <div className={`hidden sm:flex items-center justify-between mt-1.5 sm:mt-2 px-1 text-[9px] sm:text-[10px] ${isDarkMode ? "text-gray-600" : "text-gray-400"}`}>
          <span>
            {isStreaming
              ...
              : isListening
              ? "Listening... Speak now"
              : "📷 Upload images • Enter to send • Shift+Enter for new line"}
          </span>
          <span aria-live="polite">{message.length > 0 ? `${message.length} chars` : ""}</span>
        </div>
      </form>
    </div>
  );
