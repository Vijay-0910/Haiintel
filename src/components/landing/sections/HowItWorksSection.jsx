...
const steps = [
  {
    ...
    description: "Sign up and configure your AI assistant with your brand voice and knowledge base",
    iconPath: "M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z",
  },
  {
    step: "02",
    title: "Add to Website",
    description: "Copy and paste a single line of code into your website - works everywhere",
...
const StepCard = memo(({ item, isDarkMode, isLast }) => (
  <div className="relative text-center">
    ...
    <h3 className={`text-xl sm:text-2xl font-bold mb-4 ${isDarkMode ? "text-haiintel-text" : "text-gray-900"}`}>
      {item.title}
    </h3>
    ...
    {/* Connector Line */}
    {!isLast && (
      <div className={`hidden md:block absolute top-10 left-[60%] w-[80%] h-0.5 bg-gradient-to-r ${isDarkMode ? "from-haiintel-cyan/50" : "from-haiintel-blue/30"} to-transparent`} />
    )}
  </div>
));
...
const HowItWorksSection = memo(({ isDarkMode }) => (
  <section
    id="dos"
    className={`py-20 sm:py-24 px-4 sm:px-6 relative overflow-hidden transition-colors duration-200 ${isDarkMode ? "bg-haiintel-dark" : "bg-white"}`}
    style={{ contentVisibility: "auto", containIntrinsicSize: "0 700px" }}
  >
    {/* Background */}
    <div className={`absolute inset-0 ${isDarkMode ? "bg-gradient-to-r from-haiintel-blue/5 to-haiintel-accent/5" : "bg-gradient-to-r from-blue-50 to-orange-50"}`} />

    <div className="container mx-auto max-w-6xl relative z-10">
      {/* Header */}
      <div className="text-center mb-12 sm:mb-16">
        <motion.span
          className={`text-sm font-semibold tracking-wider uppercase mb-4 block ${isDarkMode ? "text-haiintel-accent" : "text-orange-500"}`}
